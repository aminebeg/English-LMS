        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <a href="{{ route('tests.show', $test) }}"
                    class="inline-flex items-center text-sm text-gray-500 hover:text-indigo-600 hover:text-indigo-500 mb-4 transition-colors">
                ...
                <p class="text-gray-600">Update details for <span
                ...
                <form method="POST" action="{{ route('tests.update', $test) }}" class="p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" id="passing_score" name="passing_score"
                                    value="{{ old('passing_score', $test->passing_score) }}" min="0"
                                    max="100" required
                        ...
                        <div>
                            ...
                            <input type="number" id="order" name="order" value="{{ old('order', $test->order) }}"
                                required min="1"
                                class="w-full rounded-md border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <select id="type" name="type"
                                class="w-full rounded-md border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                                <option value="quiz" {{ old('type', $test->type) == 'quiz' ? 'selected' : '' }}>Quiz
                                </option>
                                <option value="final_exam"
                                    {{ old('type', $test->type) == 'final_exam' ? 'selected' : '' }}>Final Exam</option>
                            </select>
                        </div>
                            ...
                            <select id="course_section_id" name="course_section_id"
                                ...
                                @foreach ($test->course->sections as $section)
                                    <option value="{{ $section->id }}"
                                        {{ old('course_section_id', $test->course_section_id) == $section->id ? 'selected' : '' }}>
                                        {{ $section->title }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="lesson_id" name="lesson_id"
                                ...
                                @foreach ($test->course->lessons as $lesson)
                                    <option value="{{ $lesson->id }}"
                                        {{ old('lesson_id', $test->lesson_id) == $lesson->id ? 'selected' : '' }}>
                                        {{ $lesson->title }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                        <a href="{{ route('tests.show', $test) }}"
                            class="text-sm font-medium text-gray-600 hover:text-gray-900 hover:text-gray-900 transition-colors">
                            Cancel
                        </a>
