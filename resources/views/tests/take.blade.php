                            ...
                            @foreach($questions as $index => $question)
                                <div class="border-b border-gray-700 pb-6 last:border-0">
                                    <div class="flex items-start gap-3 mb-4">
                                        <span class="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-indigo-100 text-indigo-800 bg-indigo-900 text-indigo-200 font-bold text-sm">
                                            {{ $index + 1 }}
                                        </span>
                                        <div class="flex-grow">
                                        ...
                                        @if($question->type === 'multiple_choice')
                                            <div class="space-y-3">
                                                @foreach($question->options as $option)
                                                    <label class="flex items-center space-x-3 cursor-pointer p-3 rounded-lg hover:bg-gray-50 hover:bg-gray-700 transition">
                                                        <input type="radio" 
                                        ...
                                        @elseif($question->type === 'short_answer')
                                            <div class="mt-2">
                                                <textarea name="answers[{{ $question->id }}]" 
                                                          rows="2" 
                                                          class="block w-full rounded-md border-gray-300 border-gray-700 bg-gray-900 text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" 
