<x-app-layout>
    <div class="min-h-screen bg-gray-50 py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-600">Add a question to <span
                        class="font-semibold text-indigo-600">{{ $test->title }}</span></p>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <form method="POST" action="{{ route('questions.store') }}" id="questionForm" class="p-8">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <label class="cursor-pointer relative">
                                ...
                                <div
                                    class="p-4 rounded-lg border border-gray-200 hover:border-indigo-500 peer-checked:border-indigo-600 peer-checked:bg-indigo-50 peer-checked:bg-indigo-50 transition-all text-center">
                            ...
                            <label class="cursor-pointer relative">
                                <input type="radio" name="type" value="short_answer" class="peer sr-only"
                                    {{ old('type') === 'short_answer' ? 'checked' : '' }}
                                    onchange="updateQuestionType()">
                                <div
                                    class="p-4 rounded-lg border border-gray-200 hover:border-indigo-500 peer-checked:border-indigo-600 peer-checked:bg-indigo-50 peer-checked:bg-indigo-50 transition-all text-center">
                                    <div class="text-2xl mb-2">✍️</div>
                                    <div class="font-semibold text-gray-800">Short Answer</div>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <button type="button" id="generate-question"
                                class="inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-700 hover:text-indigo-300 transition-colors">
                        ...
                        <textarea id="content" name="content" rows="3" required
                            class="w-full rounded-md border-gray-300 bg-white text-gray-800 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm text-base"
                    ...
                    <div id="multipleChoiceSection" class="mb-8 hidden">
                        ...
                        <div
                            class="bg-gray-50 rounded-lg p-6 space-y-4 border border-gray-100">
                            <div id="optionsContainer" class="space-y-3">
                                @for ($i = 0; $i < 4; $i++)
                                    <div class="flex items-center gap-3 option-row">
                                        <div
                                            class="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-sm">
                                            {{ chr(65 + $i) }}
                                        </div>
                                        <input type="text" name="options[]" value="{{ old('options.' . $i) }}"
                                            placeholder="Option {{ $i + 1 }}"
                                            class="flex-1 rounded-md border-gray-300 bg-white text-gray-800 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                                        <input type="radio" name="correct_option_radio" value="{{ $i }}"
                                            ...
                                            onclick="setCorrectAnswer(this)" title="Mark as correct answer">
                                    </div>
                                @endfor
                            </div>
                        ...
                        <p class="mt-2 text-xs text-gray-500">Click the radio button next to an option to mark it as
                    ...
                    <div id="trueFalseSection" class="mb-8 hidden">
                        ...
                        <div class="flex gap-4">
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="tf_radio" value="True" class="peer sr-only"
                                    onchange="setTFAnswer('True')">
                                <div
                                    class="p-4 rounded-lg border border-gray-200 hover:border-green-500 peer-checked:border-green-600 peer-checked:bg-green-50 peer-checked:bg-green-900/20 transition-all text-center">
                                    <span class="font-bold text-lg text-green-700">True</span>
                                </div>
                            </label>
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="tf_radio" value="False" class="peer sr-only"
                                    onchange="setTFAnswer('False')">
                                <div
                                    class="p-4 rounded-lg border border-gray-200 hover:border-red-500 peer-checked:border-red-600 peer-checked:bg-red-50 peer-checked:bg-red-900/20 transition-all text-center">
                    ...
                    <div id="shortAnswerSection" class="mb-8 hidden">
                        ...
                        <input type="text" id="short_answer_input"
                            class="w-full rounded-md border-gray-300 bg-white text-gray-800 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm"
                            placeholder="Enter the expected answer..."
                            oninput="document.getElementById('correct_answer').value = this.value">
                        <p class="mt-2 text-xs text-gray-500">Students must match this answer exactly
                            (case-insensitive).</p>
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <input type="number" id="order" name="order"
                            value="{{ old('order', $test->questions->count() + 1) }}" required min="1"
                            class="w-32 rounded-md border-gray-300 bg-white text-gray-800 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                    </div>

                    <!-- Action Buttons -->
                    <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                        <a href="{{ route('tests.show', $test) }}"
                            class="text-sm font-medium text-gray-600 hover:text-gray-800 hover:text-gray-900 transition-colors">
                            Cancel
                        </a>
    ...
    <script>
        ...
        document.getElementById('generate-question').addEventListener('click', function() {
            const type = document.querySelector('input[name="type"]:checked').value;
            const prompt =
                `Generate a challenging ${type} question for an English test. Return ONLY the question text.`;
