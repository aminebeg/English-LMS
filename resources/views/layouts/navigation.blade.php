<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->hasRole('student'))
                        <x-nav-link :href="route('courses.browse')" :active="request()->routeIs('courses.browse')">
                            {{ __('Browse Courses') }}
                        </x-nav-link>
                        <x-nav-link :href="route('enrollments.index')" :active="request()->routeIs('enrollments.*')">
                            {{ __('My Courses') }}
                        </x-nav-link>
                        <x-nav-link :href="route('tests.my-results')" :active="request()->routeIs('tests.my-results')">
                            {{ __('Test Results') }}
                        </x-nav-link>
                    ...
                    @if(Auth::user()->hasRole('tutor'))
                        <x-nav-link :href="route('courses.index')" :active="request()->routeIs('courses.*') || request()->routeIs('lessons.*') || request()->routeIs('materials.*') || request()->routeIs('tests.*') || request()->routeIs('questions.*')">
                            {{ __('My Courses') }}
                        </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->hasRole('student'))
                <x-responsive-nav-link :href="route('courses.browse')" :active="request()->routeIs('courses.browse')">
                    {{ __('Browse Courses') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('enrollments.index')" :active="request()->routeIs('enrollments.*')">
                    {{ __('My Courses') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('tests.my-results')" :active="request()->routeIs('tests.my-results')">
                    {{ __('Test Results') }}
                </x-responsive-nav-link>
