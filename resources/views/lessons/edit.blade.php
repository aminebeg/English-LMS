                ...
                <div class="grid grid-cols-12 gap-8">
                    ...
                    <div class="col-span-12 lg:col-span-8 space-y-6">
                        ...
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            <div class="space-y-6">
                                <div>
                                    <label for="title" class="block text-sm font-medium text-gray-700">Lesson Title
                                        <span class="text-red-500">*</span></label>
                                    <input type="text" name="title" id="title" value="{{ old('title', $lesson->title) }}"
                                        required placeholder="e.g., Introduction to Grammar"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-lg bg-white border-gray-300 text-gray-900">
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="summary" id="summary" rows="2" placeholder="Brief overview..."
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-white border-gray-300 text-gray-900">{{ old('summary', $lesson->summary) }}</textarea>
                                </div>
                        ...
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100">
                                <p class="text-xs font-medium text-gray-500 mb-3 uppercase tracking-wider">Add Content
                                    Block</p>
                                <div class="flex flex-wrap gap-2">
                                    <button type="button" onclick="addBlock('heading')"
                                        class="flex items-center gap-2 px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-md text-sm text-gray-700 transition-colors border border-gray-200">
                        ...
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            ...
                            <div class="mb-6">
                                ...
                                <button type="button" id="add-vocabulary-btn"
                                    ...
                                    Word</button>
                                <input type="hidden" name="vocabulary" id="vocabulary-hidden" value="{{ json_encode(old('vocabulary', $lesson->vocabulary ?? [])) }}">
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="exercise-input" rows="2" placeholder="Describe an exercise..."
                                    class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-white border-gray-300 text-gray-900"></textarea>
                                <button type="button" id="add-exercise-btn"
                                    class="mt-2 w-full sm:w-auto px-3 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 border border-gray-300 text-sm font-medium">Add
                                    Exercise</button>
                                <input type="hidden" name="exercises" id="exercises-hidden" value="{{ json_encode(old('exercises', $lesson->exercises ?? [])) }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-span-12 lg:col-span-4 space-y-6">
                        ...
                        <!-- Publishing & Status -->
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Publishing
                            </h3>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-start">
                                    <div class="flex items-center h-5">
                                        <input id="is_preview" name="is_preview" type="checkbox" value="1"
                                            {{ old('is_preview', $lesson->is_preview) ? 'checked' : '' }}class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                                    </div>
                                    <div class="ml-3 text-sm">
                                ...
                                <div class="pt-4 border-t border-gray-100">
                                    <label for="course_section_id"
                                        class="block text-sm font-medium text-gray-700">Section (Optional)</label>
                                    <select name="course_section_id" id="course_section_id"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-white border-gray-300 text-gray-900">
                                        <option value="">No Section</option>
                                        @foreach ($lesson->course->sections as $section)
                                            <option value="{{ $section->id }}" {{ (old('course_section_id', $lesson->course_section_id) ?? request('course_section_id')) == $section->id ? 'selected' : '' }}>{{ $section->title }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label for="duration_minutes"
                                        class="block text-sm font-medium text-gray-700">Duration (Minutes)</label>
                                    <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', $lesson->duration_minutes) }}"
                        ...
                        <!-- Learning Objectives -->
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Objectives
                            </h3>
                            ...
                            <input type="hidden" name="objectives" id="objectives-hidden" value="{{ json_encode(old('objectives', $lesson->objectives ?? [])) }}">
                        </div>

                        <!-- Key Points -->
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Key Points
                            </h3>
                            ...
                            <input type="hidden" name="key_points" id="keypoints-hidden" value="{{ json_encode(old('key_points', $lesson->key_points ?? [])) }}">
                        </div>

                        <!-- Resources -->
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Resources
                            </h3>
                            ...
                            <input type="hidden" name="resources" id="resources-hidden" value="{{ json_encode(old('resources', $lesson->resources ?? [])) }}">
                        </div>

                        <!-- Teacher Notes -->
                        <div class="bg-white shadow-sm rounded-lg border border-gray-200 p-6">
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Teacher Notes
                            </h3>
                            <textarea name="notes" id="notes" rows="4" placeholder="Private notes..."
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-white border-gray-300 text-gray-900">{{ old('notes', $lesson->notes ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const templates = {
                heading: (id) => `
                    <div class="flex items-center gap-4 mb-2">
                        <select class="block w-32 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-gray-900" onchange="updateHeadingLevel('${id}', this.value)">
                            <option value="h2">H2</option>
                            <option value="h3">H3</option>
                            <option value="h4">H4</option>
                        </select>
                        <input type="text" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-lg font-bold text-gray-900" placeholder="Heading Text" data-type="content">
                    </div>
                `,
                text: (id) => `
                    <div class="bg-white">
                ...
                image: (id) => `
                    <div class="space-y-3">
                        <div class="flex items-center gap-3">
                            <input type="text" id="img-url-${id}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-gray-900" placeholder="Image URL or upload a file" data-type="src">
                            <input type="file" id="img-file-${id}" accept="image/*" class="hidden" onchange="handleMediaUpload('${id}', 'image', this)">
                            <button type="button" onclick="document.getElementById('img-file-${id}').click()" class="px-3 py-2 bg-indigo-600 text-gray-900 rounded-md text-sm hover:bg-indigo-700 transition-colors whitespace-nowrap">
                        ...
                        <input type="text" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-gray-900" placeholder="Image Caption (Alt Text)" data-type="caption">
                    </div>
                `,
                video: (id) => `
                    <div class="space-y-3">
                        <div class="flex items-center gap-3">
                            <input type="text" id="vid-url-${id}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-gray-900" placeholder="Video URL (YouTube, Vimeo) or upload MP4" data-type="src">
                            <input type="file" id="vid-file-${id}" accept="video/*" class="hidden" onchange="handleMediaUpload('${id}', 'video', this)">
                            <button type="button" onclick="document.getElementById('vid-file-${id}').click()" class="px-3 py-2 bg-indigo-600 text-gray-900 rounded-md text-sm hover:bg-indigo-700 transition-colors whitespace-nowrap">
                                <svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                                Upload
                            </button>
                        </div>
                        ...
                        <div id="vid-player-${id}" class="hidden mt-2">
                            <video controls class="w-full max-h-48 rounded-lg border border-gray-200">
                ...
                code: (id) => `
                    <div class="space-y-2">
                        <select class="block w-32 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-gray-900" data-type="language">
                        ...
                        <textarea class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm font-mono text-gray-900" rows="6" placeholder="Paste code here..." data-type="code"></textarea>
                    </div>
                `,
                note: (id) => `
                    <div class="flex gap-3 p-4 bg-yellow-50 border-l-4 rounded-r-md">
                ...
                audio: (id) => `
                    <div class="space-y-3">
                        <div class="flex items-center gap-3">
                            <input type="text" id="aud-url-${id}" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-gray-900" placeholder="Audio URL or upload a file" data-type="src">
                            <input type="file" id="aud-file-${id}" accept="audio/*" class="hidden" onchange="handleMediaUpload('${id}', 'audio', this)">
                            <button type="button" onclick="document.getElementById('aud-file-${id}').click()" class="px-3 py-2 bg-indigo-600 text-gray-900 rounded-md text-sm hover:bg-indigo-700 transition-colors whitespace-nowrap">
                                <svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                                Upload
                            </button>
                        </div>
            ...
            window.addBlock = function(type, data = null) {
                ...
                block.className =
                    'group relative bg-white rounded-lg border border-gray-200 p-4 hover:border-indigo-300 hover:border-indigo-700 transition-all';
                block.dataset.id = id;
                block.dataset.type = type;

                block.innerHTML = `
                    <div class="absolute left-0 top-0 bottom-0 w-8 flex items-center justify-center cursor-move block-handle opacity-0 group-hover:opacity-100 transition-opacity bg-gray-50 rounded-l-lg border-r border-gray-200">
                    ...
                    <div class="pl-6 pr-8">
                        <div class="mb-2 flex items-center justify-between">
                            ...
                            <button type="button" onclick="removeBlock('${id}')" class="text-gray-500 hover:text-red-500 transition-colors">
            ...
            document.getElementById('lessonForm').addEventListener('submit', function(e) {
                ...
                if (blocks.length === 0) {
                    e.preventDefault();
                    alert(
                        '❌ Please add at least one content block to your lesson!\n\nYour lesson needs content for students to learn from.'
                        );
                    return false;
                }
            ...
            const initList = (id, hiddenId, template, addBtnId, onAdd) => {
                ...
                const render = () => {
                    ...
                    items.forEach((item, index) => {
                        ...
                        div.className = 'flex justify-between items-center gap-2 mb-2 p-2 bg-gray-50 rounded border';
                        div.innerHTML = `
                            <div class="flex-grow">${template(item, index)}</div>
                            <button type="button" class="text-red-500 hover:text-red-700 px-2 flex-shrink-0" onclick="removeItem('${id}', ${index})">×</button>
                        `;
            ...
            initList('vocabulary-list', 'vocabulary-hidden', 
                (item) => `<strong>${item.word}</strong>: ${item.definition}`, 
                'add-vocabulary-btn', 
                () => {
                    const word = document.getElementById('vocab-word-input');
                    const def = document.getElementById('vocab-definition-input');
                    if (word.value && def.value) {
                        const item = { word: word.value, definition: def.value };
                        word.value = ''; def.value = '';
                        return item;
                    }
                    return null;
                }
            ...
            initList('exercises-list', 'exercises-hidden', 
                (item) => `${item}`, 
                'add-exercise-btn', 
                () => {
            ...
            initList('objectives-list', 'objectives-hidden', 
                (item) => `${item}`, 
                'add-objective-btn', 
                () => {
            ...
            initList('keypoints-list', 'keypoints-hidden', 
                (item) => `${item}`, 
                'add-keypoint-btn', 
                () => {
            ...
            initList('resources-list', 'resources-hidden', 
                (item) => `<a href="${item.url}" target="_blank" class="text-blue-500 hover:underline">${item.title}</a>`, 
                'add-resource-btn', 
                () => {
                    const title = document.getElementById('resource-title-input');
                    const url = document.getElementById('resource-url-input');
                    if (title.value && url.value) {
                        const item = { title: title.value, url: url.value };
            ...
            function showNotification(message, type = 'info') {
                const bgColor = type === 'success' ? 'bg-green-500' : type === 'error' ? 'bg-red-500' :
                    'bg-blue-500';
                const notification = document.createElement('div');
                notification.className =
                    `fixed bottom-4 right-4 ${bgColor} text-gray-900 px-6 py-3 rounded-lg shadow-lg z-50 transition-opacity duration-300`;
