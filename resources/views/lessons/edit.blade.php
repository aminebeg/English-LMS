<x-app-layout>
                    ...
                    <div class="col-span-12 lg:col-span-8 space-y-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                            ...
                            <textarea name="content" id="content" rows="20" required
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm font-mono dark:bg-gray-900 dark:border-gray-600 dark:text-white">{{ old('content', $lesson->content) }}</textarea>
                        </div>
    ...
    <script>
        // Initialize Data
        let objectives = JSON.parse(document.getElementById('objectives-hidden').value || '[]');
        let keyPoints = JSON.parse(document.getElementById('keypoints-hidden').value || '[]');
        let vocabulary = JSON.parse(document.getElementById('vocabulary-hidden').value || '[]');
        let exercises = JSON.parse(document.getElementById('exercises-hidden').value || '[]');
        let resources = JSON.parse(document.getElementById('resources-hidden').value || '[]');
