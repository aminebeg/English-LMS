                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('courses.show', $lesson->course) }}"
                                class="ml-1 text-sm font-medium text-gray-600 hover:text-indigo-600 transition-colors">{{ $lesson->course->title }}</a>
                        </div>
                    </li>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                        <div class="p-8">
                            <div class="flex justify-between items-start mb-6">
                                <div>
                                    <div class="flex items-center gap-3 mb-2">
                                        <span
                                            class="px-2.5 py-0.5 bg-indigo-100 text-indigo-800 text-xs font-bold rounded-full">
                                            Lesson {{ $lesson->order }}
                                        </span>
                                        @if ($lesson->difficulty)
                                            <span
                                                class="px-2.5 py-0.5 bg-purple-100 text-purple-800 text-xs font-bold rounded-full">
                                                {{ ucfirst($lesson->difficulty) }}
                                            </span>
                                        @endif
                                        @if ($lesson->duration_minutes)
                                            <span
                                                class="px-2.5 py-0.5 bg-blue-100 text-blue-800 text-xs font-bold rounded-full flex items-center gap-1">
                                ...
                                @can('update', $lesson->course)
                                    <div class="flex gap-2">
                                        ...
                                        <form method="POST" action="{{ route('lessons.destroy', $lesson) }}"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="p-2 text-gray-500 hover:text-red-600 hover:text-red-400 transition-colors">
                            ...
                            @if ($lesson->summary)
                                <div
                                    class="p-4 bg-gray-50 rounded-lg mb-8 text-gray-700 italic border-l-4">
                                    {{ $lesson->summary }}
                                </div>
                            @endif
                                ...
                                <div class="space-y-6 text-gray-900">
                                    @foreach ($contentBlocks as $block)
                                        @switch($block['type'])
                                            @case('heading')
                                                <{{ $block['data']['level'] ?? 'h2' }}
                                                    class="font-bold text-gray-900 {{ ($block['data']['level'] ?? 'h2') === 'h2' ? 'text-2xl mt-8 mb-4' : (($block['data']['level'] ?? 'h2') === 'h3' ? 'text-xl mt-6 mb-3' : 'text-lg mt-4 mb-2') }}">
                                                    {{ $block['data']['content'] ?? '' }}
                                                    </{{ $block['data']['level'] ?? 'h2' }}>
                                                @break
                                                ...
                                                @case('image')
                                                    ...
                                                    <figure class="my-6">
                                                        <img src="{{ $src }}"
                                                            ...
                                                            class="rounded-lg shadow-sm w-full object-cover max-h-[500px]">
                                                ...
                                                @case('video')
                                                    ...
                                                    @endphp
                                                    <div
                                                        class="my-6 aspect-video rounded-lg overflow-hidden shadow-sm border border-gray-200 bg-black">
                                                        @if ($embedUrl)
                                                ...
                                                @case('code')
                                                    <div class="my-6 relative group">
                                                        <div
                                                            class="absolute top-0 right-0 px-2 py-1 text-xs font-mono bg-gray-800 rounded-bl-md">
                                                ...
                                                @case('note')
                                                    <div
                                                        class="my-6 flex gap-4 p-4 bg-yellow-50 border-l-4 rounded-r-lg">
                                                        <div class="flex-shrink-0">
                                                            <svg class="h-6 w-6 text-yellow-400" fill="none"
                                    ...
                                    <div class="p-4 space-y-4">
                                        @foreach ($lesson->exercises as $index => $exercise)
                                            <div class="flex gap-3">
                                                <span
                                                    class="flex-shrink-0 w-5 h-5 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center text-xs font-bold">
                                                    {{ $index + 1 }}
                                                </span>
                ...
                <div class="space-y-6">
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                            ...
                            <div class="space-y-3">
                                @foreach ($lesson->materials->where('type', 'file') as $material)
                                    <div
                                        class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100 group hover:border-indigo-300 transition-all">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="p-2 bg-indigo-100 rounded-lg text-indigo-600 group-hover:scale-110 transition-transform">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                            ...
                                            <div>
                                                <div class="text-sm font-bold text-gray-900">{{ $material->title }}
                                                </div>
                                                <div class="text-xs text-gray-500">
                                                    {{ strtoupper(pathinfo($material->file_name, PATHINFO_EXTENSION)) }}
                                                    • {{ number_format($material->file_size / 1024, 1) }} KB</div>
                                            </div>
                                        </div>
                                        <a href="{{ route('materials.download', $material) }}"
                                            class="flex items-center justify-center w-8 h-8 rounded-full bg-white hover:text-indigo-600 hover:shadow-sm transition-all">
                            ...
                            <div class="space-y-3">
                                @foreach ($lesson->resources as $resource)
                                    <a href="{{ $resource['url'] }}" target="_blank"
                                        class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 hover:bg-gray-700 transition-colors group border border-gray-100">
                                        <div
                                            class="w-8 h-8 bg-white rounded-md flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                                            <img src="https://www.google.com/s2/favicons?domain={{ parse_url($resource['url'], PHP_URL_HOST) }}"
                                                alt="" class="w-4 h-4">
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="text-xs text-gray-500 truncate">
                                                {{ parse_url($resource['url'], PHP_URL_HOST) }}</div>
                                        </div>
                                    </a>
                                @endforeach
                            </div>
                    ...
                    @can('update', $lesson->course)
                        @if ($lesson->notes)
                            <div
                                class="bg-yellow-50 border border-yellow-200 rounded-lg shadow-sm p-6">
